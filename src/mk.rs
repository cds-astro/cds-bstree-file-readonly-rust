...
pub struct MkIndex<R> where R: Read {
  reader: Reader<R>,
...
impl <R> MkIndex<R> where R: Read {
  ...
  pub fn new<S: Read>(reader: Reader<S>, col_indices: ColIndices, supports_null: bool, args: MkAlgoArgs, mem_args: MemSizeArgs) -> MkIndex<S> {
    MkIndex {
      reader, col_indices, supports_null, args, mem_args
    }
  }
  
  fn mk_no_null<I, V, IRW, VRW, P>(mut self, types: &IdVal, id_rw: &IRW, val_rw: &VRW, csv2entry: P) -> Result<<Self as Process>::Output, std::io::Error> //Self::Output
    where I: Id,   // Identifier type (from u64 if recno option)
    ...
    for chunk in &self.reader.records().enumerate().chunks(self.args.chunk_size) {
      let mut entries: Vec<Entry<I, V>> = chunk
        .map(|(i, rec_res)| rec_res.map_err(to_io_err).and_then(|rec| csv2entry(i, &rec)))
  ...
  fn mk_with_null<I, V, IRW, VRW, P>(self, _types: &IdVal, _id_rw: &IRW, _val_rw: &VRW, _csv2entry: P)
...
fn get_with_err<F: FromStr>(record: &StringRecord, index: usize, col_name: &'static str) -> Result<F, Error> {
  let res = record.get(index);
  match res {
    Some(str_ref) => {
      if str_ref.is_empty() {
